...
class FormComparison:
    ...
    def align_poses(self, user_landmarks, ideal_landmarks):
        ...
        # Apply base scaling and smoothing
        base_scale = 1.8  # Increased base scale to make ideal form larger
        
        # Calculate new scales with smoothing
        new_vertical_scale = height_scale * base_scale
        new_horizontal_scale = width_scale * base_scale * 0.6  # Keep width compression
        
        # Apply smoothing between frames
        vertical_scale = (self.smoothing_factor * new_vertical_scale + 
                         (1 - self.smoothing_factor) * self.last_vertical_scale)
        horizontal_scale = (self.smoothing_factor * new_horizontal_scale + 
                          (1 - self.smoothing_factor) * self.last_horizontal_scale)
        
        # Update last scales for next frame
        self.last_vertical_scale = vertical_scale
        self.last_horizontal_scale = horizontal_scale
        
        # Tighter scale limits for more stability
        vertical_scale = max(1.4, min(vertical_scale, 1.9))  # Increased vertical scale limits
        horizontal_scale = max(0.8, min(horizontal_scale, 1.3))  # Increased horizontal scale limits
